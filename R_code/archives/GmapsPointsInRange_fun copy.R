...
GmapsPointsInRange <- function(hubs.data, points.data, constraint,
  ...
  # points.data <- readRDS("../output/IOWA_wtd_cntroids.sptdf.RDS")
  # constraint = "distance"
  # max.dist = 50
  
  ###### LOAD LIBRARIES #######
  packages <- c("ggmap", "raster", "sp", "ggplot2", "rgeos", "spRbind",
                "spatialEco")
  ...
  # init distances vector to store all route distances
  distances.v <- c(NULL)
  ...
  for (RID in hubs.data$RID){
    ...
    for (fips in cand.counties$FIPS){
      ...
      # add distance to vector of distances
