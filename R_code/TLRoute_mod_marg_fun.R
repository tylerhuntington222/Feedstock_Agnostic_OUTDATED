...
TLRoute <- function(basemap, start.coords, end.coords) {
  ...
  # create spatial points obj to store start and end
  ...
  # create bbox to crop basemap to smaller area
  marg <- 0.01
  ...
  # crop basemap to route area
  basemap <- raster::crop(orig.bmap, box)
  plot(basemap, col = 'grey')
  plot(start.end.pts, col = 'blue', add = T, pch = 16)
...
par.state.counties <- counties[counties$STATEABBREV == "SD",]

# subset for refinery of interest
par.ref <- biorefs[biorefs$RID == 131,]
...
#for (c in state.cents$NAME) {
  ...
  print(paste0("working on county: ", c))
  par.cnty = cents[cents$NAME == c,]
