...
CalcCropClusterCents <- function(raster.path, counties.data, crop) {
  ...
  not.done <- fips.codes[!(fips.codes %in% done)]
  ...
  ded.rows <- which(ras.vals.df$Class_Names == crop)
  ded.vals <- ras.vals.df[ded.rows, "ID"]
  
  # find raster vals for cells with dual production of target crop + other
  dbl.rows <- grep(crop, ras.vals.df$Class_Names)
  dbl.rows <- dbl.rows[!(dbl.rows %in% ded.rows)]
  dbl.vals <- ras.vals.df[dbl.rows, "ID"]
  ...
  US.cluster.cents <-
    foreach(fips = fips.codes[1:length(not.done)],
            .combine = "rbind",
...
# ###### USAGE ######
# test function on Corn
raster.path <- "../../Desktop/very_large_files//cdl/cdl_2016_30m.img"
counties <- readRDS("../clean_binary_data/counties.spdf.RDS")
crop <- "Corn"
corn_kmeans <- CalcCropClusterCents(raster.path, counties.data, crop)
