...
parMqPointsInRange <- function(hubs.data, nodes.data, constraint, 
  ...
  ###### LOAD LIBRARIES #######
  packages <- c("ggmap", "raster", "sp", "ggplot2", "foreach",
  ...
  # initiatialize cluster
  num.cores <- detectCores()
  cl <- makeCluster(num.cores)
  registerDoParallel(cl)
  
  # TEMP: constrain RID sequence
  res.list <- foreach (RID = hubs.data$RID[1:length(hubs.data$RID)], 
                       .packages = "sp", 'raster') %dopar% {
    
    # source the routing function
    source("MqRoute_fun.R")
    ...
    cand.counties <- raster::crop(nodes.data, buff)
    ...
    for (fips in cand.counties$FIPS){
      ...
      # pause to comply with query rate limit
      Sys.sleep(0.5) 
